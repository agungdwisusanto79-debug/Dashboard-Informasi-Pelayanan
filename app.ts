...
import { usersTable, reportsTable, auditLogsTable, hashPassword, DEFAULT_DEMO_PASSWORD_HASH, DBUser } from './db';
...
// Global CORS, Header, and URL Normalization Middleware
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  // Handle URL normalization for Vercel Serverless Function rewrites
  const matchedPath = req.headers['x-matched-path'] as string;
  if (matchedPath && matchedPath.startsWith('/api') && (req.url === '/' || req.url === '/api' || req.url === '')) {
    req.url = matchedPath;
  }
...
// Router to handle all API logic (works with both /api/* and root rewrites on Vercel)
...
// ==========================================
apiRouter.get('/dashboard/stats', authenticate, (req: Request, res: Response) => {
  const user: DBUser = (req as any).user;
...
// ==========================================
apiRouter.get('/reports/summary', authenticate, (req: Request, res: Response) => {
  const user: DBUser = (req as any).user;
  let { startDate, endDate, district, village } = req.query as { [key: string]: string };
...
// ==========================================
apiRouter.get('/regions/jakarta-selatan', authenticate, (req: Request, res: Response) => {
...
// ==========================================
apiRouter.get('/regions/national-stats', (req: Request, res: Response) => {
  return res.json({
...
apiRouter.get('/regions/cascading', (req: Request, res: Response) => {
...
// 2. Add New Officer
apiRouter.post('/admin/users', authenticate, (req: Request, res: Response) => {
...
apiRouter.put('/admin/users/:id', authenticate, (req: Request, res: Response) => {
  const adminUser: DBUser = (req as any).user;
  if (adminUser.role !== 'SUDIN') {
...
apiRouter.patch('/admin/users/:id/status', authenticate, (req: Request, res: Response) => {
  const adminUser: DBUser = (req as any).user;
  if (adminUser.role !== 'SUDIN') {
...
apiRouter.post('/admin/users/:id/reset-password', authenticate, (req: Request, res: Response) => {
  const adminUser: DBUser = (req as any).user;
  if (adminUser.role !== 'SUDIN') {
...
apiRouter.get('/admin/audit-logs', authenticate, (req: Request, res: Response) => {
  ...
  return res.json({ auditLogs: auditLogsTable.slice(0, 100) });
});

// Health check endpoint
apiRouter.get('/health', (req: Request, res: Response) => {
  return res.json({ status: 'ok', service: 'DIP Sudin Dukcapil Jaksel API' });
});

// Mount router on /api prefix and root to support Vercel serverless functions rewrites
app.use('/api', apiRouter);
app.use(apiRouter);

// Strict JSON 404 handler for any unmatched /api route - NEVER return HTML for API routes!
app.all('/api/*', (req: Request, res: Response) => {
  res.status(404).json({
    error: `Endpoint API '${req.method} ${req.originalUrl || req.url}' tidak ditemukan.`
  });
});

// Strict JSON Error Handler
app.use((err: any, req: Request, res: Response, next: NextFunction) => {
  console.error('[API Server Error]:', err);
  res.status(500).json({
    error: err?.message || 'Terjadi kesalahan internal pada server API.'
  });
});
